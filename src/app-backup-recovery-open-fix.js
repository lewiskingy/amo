...
(function initBackupRecoveryOpenCompatibility(){
  ...
  async function retain(r,now=new Date()){
    const entries=(await r.listBackups()).map(name=>({name,date:parse(name),weekly:/^\d{4}-\d{2}-\d{2}$/.test(name)})).filter(x=>x.date).sort((a,b)=>a.date-b.date),cutoff=weekStart(now);cutoff.setDate(cutoff.getDate()-49);const keep=new Set(),weeks=new Map(),months=new Set();
    for(const e of entries){if(e.date>=cutoff){const key=day(weekStart(e.date)),existing=weeks.get(key);if(!existing||(!existing.weekly&&e.weekly)){if(existing)keep.delete(existing.name);weeks.set(key,e);keep.add(e.name)}continue}const key=`${e.date.getFullYear()}-${pad(e.date.getMonth()+1)}`;if(!months.has(key)){months.add(key);keep.add(e.name)}}
