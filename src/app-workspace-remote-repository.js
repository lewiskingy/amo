...
(function initRemoteWorkspaceRepository(){
  class RemoteWorkspaceRepository extends WorkspaceRepository{
    ...
    async request(path,options={}){
      ...
      let token=null;
      try{token=await window.amoAuth?.getApiToken?.({interactive:false})||null}catch(e){console.warn('AMO could not acquire an API access token silently.',e)}
      ...
      if(!response.ok){
        const message=response.status===401&&!window.amoAuth?.isSignedIn?.()
