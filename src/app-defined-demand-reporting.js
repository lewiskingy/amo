...
(function initDefinedDemandReporting(){
  ...
  async function ensurePlanningReporting(){
    try{
      if(!window.PlanningReporting)await loadScript('app-planning-reporting.js','amoPlanningReporting');
      if(!window.PlanningReportingUI)await loadScript('app-planning-reporting-ui.js','amoPlanningReportingUi');
      if(!window.PlanningReportingHooks)await loadScript('app-planning-reporting-hooks.js','amoPlanningReportingHooks')
    }catch(error){console.warn('Planning reporting integration unavailable.',error)}
  }
