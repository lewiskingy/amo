...
(function initFinancialPlanning(){
  ...
  if(typeof DEFAULT_SETTINGS!=='undefined'&&!DEFAULT_SETTINGS.financialPlanning)DEFAULT_SETTINGS.financialPlanning={defaultDayRate:DEFAULT_DAY_RATE};
  ...
  function ensureFinancialDefaults(){if(!workspaceHandle||!db?.settings)return false;const n=Number(db.settings?.financialPlanning?.defaultDayRate);if(Number.isFinite(n)&&n>0)return false;db.settings.financialPlanning={...(db.settings.financialPlanning||{}),defaultDayRate:DEFAULT_DAY_RATE};db.configFiles=db.configFiles||{};db.configFiles['settings.json']=clone(db.settings);configDirty=true;updateBanner?.();requestAutosave?.();log?.(`Bootstrapped Financial Planning default day rate at ${money(DEFAULT_DAY_RATE)} / day.`);return true}
  ...
  if(typeof renderConfig==='function'){
    ...
    renderConfig=function(){ensureFinancialDefaults();const result=baseRenderConfigFinance();const state=window.AMO_CONFIG_PAGE_STATE,content=$('configContent');if(!workspaceHandle||!state||state.activeTab!=='system'||!content)return result;if(content.querySelector('.financial-planning-card'))return result;const editing=state.editingTab==='system',settings=editing?state.draft:db.settings,rate=defaultDayRate(settings),grid=content.querySelector('.settings-grid');if(!grid)return result;const card=document.createElement('div');card.className='card financial-planning-card';card.innerHTML=`<div class="section-title" style="margin-top:0"><div><h2>Financial Planning</h2><p class="muted config-description">Planning charge rate used to convert estimates and FTE allocations into indicative £ values.</p></div></div><div class="settings-field"><label>Default billable day rate (£)</label>${editing?`<input class="cell-input" id="settingsDefaultDayRate" type="number" min="1" step="25" value="${rate}">`:`<strong>${money(rate)} / day</strong>`}</div><div class="settings-note">People may optionally carry their own day-rate override. Planning months use Monday–Friday working days.</div>`;grid.appendChild(card);$('settingsDefaultDayRate')?.addEventListener('input',e=>{state.draft.financialPlanning=state.draft.financialPlanning||{};state.draft.financialPlanning.defaultDayRate=Math.max(1,Number(e.target.value)||DEFAULT_DAY_RATE)});return result
    };
  }

  /* Demand and People list metadata. The live Demand register uses demandTailColumns from app-integrations. */
  ...
  if(typeof teamCols!=='undefined'&&!teamCols.some(c=>c.key==='billingDayRate')){const fteIndex=teamCols.findIndex(c=>c.key==='fte');teamCols.splice(Math.max(0,fteIndex+1),0,{key:'billingDayRate',label:'Day Rate £',type:'number',editable:true})}
  ...
  if(workspaceHandle){ensureFinancialDefaults();if(typeof refreshAll==='function')refreshAll()}
})();
