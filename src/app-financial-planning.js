...
(function initFinancialPlanning(){
  ...
  if(typeof integratedDemandCell==='function'){
    const baseIntegratedDemandCellFinance=integratedDemandCell;integratedDemandCell=function(row,col){if(col.key==='_estimatedValue'){const e=estimateInfo(row);return e.days?`<span class="nowrap">${e.kind==='ROM'?'~':''}${moneyCompact(estimatedValue(row))}</span>`:'<span class="muted">—</span>'}return baseIntegratedDemandCellFinance(row,col)};
  }
