/* Authoritative AMO Users administration and minimal client-side RBAC.
   User <-> Person mapping is part of the same explicit Users edit transaction. Person.userId is
   the relationship source of truth; selecting a Person only changes the draft until Save Users. */
(function initUsersAdmin(){
  const state={editing:false,draft:[],personLinks:{},personBaseline:{},saving:false};
  ...
  const people=()=>{try{return Array.isArray(db?.team)?db.team:[]}catch{return[]}};
  ...
  const capabilityLabels={
    ...
    'status.update':'Status updates','status.publish':'Publish status reports','financial.read':'Financial reporting','system.configure':'Configuration',
    'users.manage':'Users & access','system.restore':'Restore / recovery'
  };

  function personLinksFromLive(){const links={};for(const person of people())if(person?.userId)links[String(person.userId)]=String(person.id);return links}
  function personForUser(userId){const personId=(state.editing?state.personLinks:personLinksFromLive())[String(userId||'')];return people().find(p=>String(p.id)===String(personId||''))||null}
  function userForPerson(personId){const links=state.editing?state.personLinks:personLinksFromLive();return Object.entries(links).find(([,id])=>String(id)===String(personId||''))?.[0]||''}

  function ensureNavigation(){
    const admin=[...document.querySelectorAll('.sidebar nav details.nav-group')].find(x=>x.querySelector(':scope > summary')?.textContent?.trim()==='Admin'||x.dataset.amoNavSection==='administration');
  ...
  function render(){
    ...
    content.innerHTML=`${claimPanel()}${identitySummary()}${claimed?`<div class="users-summary"><span class="pill blue">${rows.length} user${rows.length===1?'':'s'}</span><span class="pill">${enabled} enabled</span><span class="pill ${admins?'green':'red'}">${admins} admin${admins===1?'':'s'}</span></div><div class="notice users-note"><strong>Everyone can read.</strong> User details, access and the optional Person relationship are changed together through <strong>Edit Users</strong>. Nothing in the Person column is persisted until <strong>Save Users</strong>.</div><div class="table-wrap"><table class="users-table"><thead><tr><th>User</th><th>Company / Entra account</th><th>Google email</th><th>Google subject</th><th>Person</th><th>Access</th><th>Status</th></tr></thead><tbody>${rows.length?rows.map(rowHtml).join(''):'<tr><td colspan="7" class="muted">No Users configured.</td></tr>'}</tbody></table></div>`:''}`;
    wire()
  }
  ...
  function beginEdit(){if(state.editing)return;try{access()?.require?.('users.manage');state.editing=true;state.draft=cloneValue(currentUsers());state.personBaseline=personLinksFromLive();state.personLinks={...state.personBaseline};render()}catch(e){alert(e.message)}}
  ...
  function relationshipChanges(){
    const desiredByPerson=new Map();for(const user of state.draft){const pid=String(state.personLinks[user.id]||'');if(pid)desiredByPerson.set(pid,String(user.id))}
    return people().map(person=>({person,current:String(person.userId||''),desired:desiredByPerson.get(String(person.id))||''})).filter(x=>x.current!==x.desired)
  }
  function confirmRelationshipChanges(changes){
    const reassign=changes.filter(x=>x.current&&x.desired&&x.current!==x.desired);if(!reassign.length)return true;
    const lines=reassign.slice(0,6).map(x=>`${x.person.name||x.person.id}: ${currentUsers().find(u=>String(u.id)===x.current)?.displayName||x.current} → ${state.draft.find(u=>String(u.id)===x.desired)?.displayName||x.desired}`);
    return confirm(`Saving will reassign ${reassign.length} existing Person/User relationship${reassign.length===1?'':'s'}:\n\n${lines.join('\n')}${reassign.length>6?'\n…':''}\n\nContinue?`)
  }
  async function persistRelationships(repo,changes){
    const saved=[];
    try{for(const change of changes){const before=cloneValue(change.person),next=cloneValue(change.person);next.userId=change.desired;if(!next.userId)delete next.userId;if(next.userId&&window.amoPersonUserLink?.applyUserIdentity)window.amoPersonUserLink.applyUserIdentity(next);await repo.saveRecord('team',next);saved.push({before,next});const live=people().find(p=>String(p.id)===String(next.id));if(live)Object.assign(live,cloneValue(next))}return saved}catch(error){error.savedPeople=saved;throw error}
  }
  async function rollbackRelationships(repo,saved){for(const item of [...saved].reverse()){try{await repo.saveRecord('team',item.before);const live=people().find(p=>String(p.id)===String(item.before.id));if(live)Object.assign(live,cloneValue(item.before))}catch(_e){}}}
  async function save(){
    if(state.saving)return false;let repo=null,previousSettings=null,savedPeople=[];
    try{
      access()?.require?.('users.manage');validate();const changes=relationshipChanges();if(changes.length)access()?.require?.('people.write');if(!confirmRelationshipChanges(changes))return false;
      state.saving=true;repo=window.workspaceRepository;if(!repo)throw new Error('No workspace repository is connected.');const latest=await repo.getSettings();previousSettings=cloneValue(latest);latest.users=cloneValue(state.draft);latest.accessControl={...(latest.accessControl||{}),claimed:true,writeRequiresMapping:true};await repo.saveSettings(latest);
      try{savedPeople=await persistRelationships(repo,changes)}catch(error){savedPeople=error.savedPeople||[];throw error}
      db.settings={...db.settings,...cloneValue(latest)};state.editing=false;state.draft=[];state.personLinks={};state.personBaseline={};if(typeof log==='function')log(`Updated ${latest.users.length} AMO User profile${latest.users.length===1?'':'s'}${changes.length?` and ${changes.length} Person relationship${changes.length===1?'':'s'}`:''}.`);access()?.refresh?.();renderGrid?.('team');render();return true
    }catch(e){
      if(repo&&previousSettings){try{await rollbackRelationships(repo,savedPeople);await repo.saveSettings(previousSettings);db.settings={...db.settings,...cloneValue(previousSettings)}}catch(rollbackError){console.error('Users transaction rollback was incomplete.',rollbackError)}}
      alert(`Could not save Users: ${e.message}`);return false
    }finally{state.saving=false}
  }
  function cancel(){if(state.saving)return false;state.editing=false;state.draft=[];state.personLinks={};state.personBaseline={};render();return true}
  function wire(){
    ...
    document.querySelectorAll('[data-user-enabled]').forEach(input=>input.addEventListener('change',e=>state.draft[Number(e.target.dataset.userEnabled)].enabled=e.target.checked));document.querySelectorAll('[data-role-user]').forEach(input=>input.addEventListener('change',e=>setRole(Number(e.target.dataset.roleUser),e.target.dataset.role,e.target.checked)));document.querySelectorAll('[data-cap-user]').forEach(input=>input.addEventListener('change',e=>setCapability(Number(e.target.dataset.capUser),e.target.dataset.capability,e.target.checked)));document.querySelectorAll('[data-person-user]').forEach(select=>select.addEventListener('change',e=>setPersonLink(Number(e.target.dataset.personUser),e.target.value)));
    document.querySelectorAll('[data-user-delete]').forEach(button=>button.addEventListener('click',()=>{const user=state.draft[Number(button.dataset.userDelete)];if(user)delete state.personLinks[user.id];state.draft.splice(Number(button.dataset.userDelete),1);render()}));document.querySelectorAll('[data-use-current-google]').forEach(button=>button.addEventListener('click',()=>mapCurrentGoogle(Number(button.dataset.useCurrentGoogle))))
  }

  function registerEditTransaction(){window.AmoEditTransactions?.register?.('users',{label:'Users & Access changes',isEditing:()=>state.editing,hasUnsavedChanges:()=>state.editing,save,cancel})}
  ensureNavigation();render();registerEditTransaction();window.addEventListener('amo-edit-transactions-ready',registerEditTransaction);window.addEventListener('amo-auth-changed',render);window.addEventListener('amo-access-changed',render);setTimeout(()=>{ensureNavigation();render()},100);
  window.AmoUsersAdmin={state,render,save,cancel,beginEdit,personForUser}
})();
