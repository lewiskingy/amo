(function initActuals(){
  ...
  function aggregateWorksheet(ws,{team=[],demand=[]}={}){
    ...
    for(let r=range.s.r+1;r<=range.e.r;r++){
      ...
      const rawStaffNumber=String(get(r,'Staff Number')??'').trim(),rawName=String(get(r,'Person Name')??'').trim(),project=scope.projectByNumber.get(norm(projectNumber))||null;
  ...
  async function commitImport({analysis,fileName,worksheet}){const repo=window.workspaceRepository;if(!repo?.replaceActualsPeriods)throw new Error('The current workspace does not support Actuals imports.');const manifest={schemaVersion:FACT_SCHEMA_VERSION,source:{fileName,worksheet,staffNumberColumn:analysis.sourceHeaders?.['Staff Number']||null},import:{completedAt:new Date().toISOString(),sourceRows:analysis.stats.sourceRows,factRows:analysis.factRows,firstMonth:analysis.firstMonth,latestMonth:analysis.latestMonth,periods:analysis.months},totals:{hours:analysis.totalHours,costGbp:analysis.totalCostGbp},warnings:[analysis.stats.invalidMonthRows?`${analysis.stats.invalidMonthRows} source row(s) had an invalid Month and were ignored.`:'',analysis.stats.missingProjectRows?`${analysis.stats.missingProjectRows} source row(s) had no Project Number and were ignored.`:'',analysis.stats.ignoredPeopleRows?`${analysis.stats.ignoredPeopleRows} source row(s) did not match a current AMO Staff Number (source ${analysis.sourceHeaders?.['Staff Number']||'People # / Person #'}) or were outside project scope and were ignored.`:''].filter(Boolean)};await repo.replaceActualsPeriods(analysis.periods,manifest);window.dispatchEvent(new CustomEvent('amo:actuals-updated',{detail:{manifest}}));return manifest}
