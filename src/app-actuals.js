(function initActuals(){
  ...
  function buildScope(team=[],demand=[]){
    const personByStaffNumber=new Map(),projectByNumber=new Map();
    ...
    return{personByStaffNumber,projectByNumber,teamCount:team.length,demandCount:demand.length}
  }
  function matchPerson(get,scope){const staffNumber=String(get('Person #')??'').trim();if(staffNumber&&scope.personByStaffNumber.has(norm(staffNumber)))return{person:scope.personByStaffNumber.get(norm(staffNumber)),match:'staff-number'};return{person:null,match:null}}
  ...
  function aggregateWorksheet(ws,{team=[],demand=[]}={}){
    const {range,get}=worksheetAccessor(ws),scope=buildScope(team,demand),facts=new Map(),months=new Set(),stats={sourceRows:0,includedRows:0,ignoredPeopleRows:0,invalidMonthRows:0,missingProjectRows:0,unmatchedProjectRows:0,personlessProjectRows:0},people=new Set(),projects=new Set();let totalHours=0,totalCostGbp=0;
    for(let r=range.s.r+1;r<=range.e.r;r++){
      ...
      const rawStaffNumber=String(get(r,'Person #')??'').trim(),rawName=String(get(r,'Person Name')??'').trim(),project=scope.projectByNumber.get(norm(projectNumber))||null;
      let person=null;if(rawStaffNumber||rawName){person=matchPerson(name=>get(r,name),scope).person;if(!person){stats.ignoredPeopleRows++;continue}}else if(!project){stats.ignoredPeopleRows++;continue}else stats.personlessProjectRows++;
  ...
  async function commitImport({analysis,fileName,worksheet}){const repo=window.workspaceRepository;if(!repo?.replaceActualsPeriods)throw new Error('The current workspace does not support Actuals imports.');const manifest={schemaVersion:FACT_SCHEMA_VERSION,source:{fileName,worksheet},import:{completedAt:new Date().toISOString(),sourceRows:analysis.stats.sourceRows,factRows:analysis.factRows,firstMonth:analysis.firstMonth,latestMonth:analysis.latestMonth,periods:analysis.months},totals:{hours:analysis.totalHours,costGbp:analysis.totalCostGbp},warnings:[analysis.stats.invalidMonthRows?`${analysis.stats.invalidMonthRows} source row(s) had an invalid Month and were ignored.`:'',analysis.stats.missingProjectRows?`${analysis.stats.missingProjectRows} source row(s) had no Project Number and were ignored.`:'',analysis.stats.ignoredPeopleRows?`${analysis.stats.ignoredPeopleRows} source row(s) did not match a current AMO Staff Number (or were outside project scope) and were ignored.`:''].filter(Boolean)};await repo.replaceActualsPeriods(analysis.periods,manifest);window.dispatchEvent(new CustomEvent('amo:actuals-updated',{detail:{manifest}}));return manifest}
  ...
  window.Actuals={DEFAULTS,REQUIRED_COLUMNS,FACT_SCHEMA_VERSION,settings,normalizeMonth,inspectWorkbook,buildScope,aggregateWorksheet,analyzeWorkbook,storedSummary,replacementPreview,commitImport,clear};
})();
