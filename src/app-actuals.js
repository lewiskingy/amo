(function initActuals(){
  ...
  function personIds(person){return uniq([person?.personNumber].map(v=>String(v??'').trim()))}
  ...
  function buildScope(team=[],demand=[]){
    const personByNumber=new Map(),projectByNumber=new Map();
    for(const p of team)for(const id of personIds(p))personByNumber.set(norm(id),p);
    for(const d of demand)for(const id of projectIds(d))projectByNumber.set(norm(id),d);
    return{personByNumber,projectByNumber,teamCount:team.length,demandCount:demand.length}
  }
  function matchPerson(get,scope){const personNumber=String(get('Person #')??'').trim();if(personNumber&&scope.personByNumber.has(norm(personNumber)))return{person:scope.personByNumber.get(norm(personNumber)),match:'person-number'};return{person:null,match:null}}
  ...
  function aggregateWorksheet(ws,{team=[],demand=[]}={}){
    ...
    for(let r=range.s.r+1;r<=range.e.r;r++){
      ...
      let person=null;if(rawPerson||rawName){person=matchPerson(name=>get(r,name),scope).person;if(!person){stats.ignoredPeopleRows++;continue}}else if(!project){stats.ignoredPeopleRows++;continue}else stats.personlessProjectRows++;
  ...
  async function commitImport({analysis,fileName,worksheet}){const repo=window.workspaceRepository;if(!repo?.replaceActualsPeriods)throw new Error('The current workspace does not support Actuals imports.');const manifest={schemaVersion:FACT_SCHEMA_VERSION,source:{fileName,worksheet},import:{completedAt:new Date().toISOString(),sourceRows:analysis.stats.sourceRows,factRows:analysis.factRows,firstMonth:analysis.firstMonth,latestMonth:analysis.latestMonth,periods:analysis.months},totals:{hours:analysis.totalHours,costGbp:analysis.totalCostGbp},warnings:[analysis.stats.invalidMonthRows?`${analysis.stats.invalidMonthRows} source row(s) had an invalid Month and were ignored.`:'',analysis.stats.missingProjectRows?`${analysis.stats.missingProjectRows} source row(s) had no Project Number and were ignored.`:'',analysis.stats.ignoredPeopleRows?`${analysis.stats.ignoredPeopleRows} source row(s) did not match a current AMO People # (or were outside project scope) and were ignored.`:''].filter(Boolean)};await repo.replaceActualsPeriods(analysis.periods,manifest);window.dispatchEvent(new CustomEvent('amo:actuals-updated',{detail:{manifest}}));return manifest}
