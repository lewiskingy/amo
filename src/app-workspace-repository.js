...
(function initWorkspaceRepository(){
  ...
  class LocalWorkspaceRepository extends WorkspaceRepository{
    ...
    async writeJson(path,data){
      const h=await this.file(path,{create:true}),w=await h.createWritable();await w.write(jsonText(data));await w.close()
    }
    ...
    async connect(){
      const workspace=await this.readJson('workspace.json',{required:true});
      if(workspace.type!=='architecture-operations-hub')throw new Error('Not an Architecture Operations Hub workspace.');
      return workspace
    }
    async loadWorkspace(){
      const workspace=await this.connect();
      const [demand,team,allocations,ideas,configFiles]=await Promise.all([
        this.listJsonRecords('demand'),this.listJsonRecords('team'),this.listJsonRecords('allocations'),this.listJsonRecords('ideas',{optional:true}),this.readConfigFiles()
      ]);
    ...
    async saveChanges({workspace,settings,collections,dirty,deleted,configDirty=false}){
      if(!await this.ensureWritePermission())throw new Error('Read/write permission was not granted.');
      await this.writeJson('workspace.json',{...workspace,modifiedAt:new Date().toISOString()});
      for(const [type,folder] of Object.entries(ENTITY_FOLDERS)){
        const rows=collections[type]||[];
        for(const id of dirty[type]||[]){const rec=rows.find(x=>x.id===id);if(rec)await this.writeJson(`${folder}/${id}.json`,rec)}
        for(const id of deleted[type]||[])await this.deletePath(`${folder}/${id}.json`)
      }
      if(configDirty)await this.writeJson('config/settings.json',settings)
    }

    async listStatusReports(){
      const entries=await this.listEntries('status-reports',{optional:true});
      return entries.filter(e=>e.kind==='file'&&e.name.toLowerCase().endsWith('.json')).map(e=>e.name)
    }
    ...
    async archiveRecords(recordsByEntity){
      ...
      for(const [entity,records] of Object.entries(recordsByEntity||{})){
        for(const record of records||[]){await this.writeJson(`archive/${entity}/${record.id}.json`,record);await this.deletePath(`${entity}/${record.id}.json`)}
      }
    }
    ...
    async copyJsonDirectory(destRoot,folderName){
      let src;try{src=await this.rootHandle.getDirectoryHandle(folderName)}catch(e){if(e.name==='NotFoundError')return;throw e}
      const dest=await destRoot.getDirectoryHandle(folderName,{create:true});for await(const [name,handle] of src.entries())if(handle.kind==='file'&&name.toLowerCase().endsWith('.json'))await this.copyFile(handle,dest,name)
    }
    async createSafetyBackup({name,manifest}){
      ...
      const backups=await this.rootHandle.getDirectoryHandle('backups',{create:true}),snapshot=await backups.getDirectoryHandle(name,{create:true});
      await this.copyFile(await this.rootHandle.getFileHandle('workspace.json'),snapshot,'workspace.json');
      for(const folder of ['demand','team','allocations','ideas','config'])await this.copyJsonDirectory(snapshot,folder);
  ...
  window.workspaceRepositoryForHandle=handle=>{
    if(window.workspaceRepository?.mode==='local'&&window.workspaceRepository.nativeHandle===handle)return window.workspaceRepository;
