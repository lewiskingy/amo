...
export function renderDemandTable(host,{demands,queryService,settings,people=[],expanded,onToggle,onEdit,onEditWorkPackage,editMode=false,demandDraftFor=d=>d,workPackageDraftFor=w=>w,onInlineChange,filters={}}){
  ...
  for(const sourceDemand of demands){
    const demand=editMode?demandDraftFor(sourceDemand):sourceDemand,demandOwnSearchMatch=queryService.demandMatchesSearch(sourceDemand,filters.search),workPackages=queryService.workPackagesFor(sourceDemand.id,{show:filters.show||'active',control:filters.control||'',search:filters.search||'',filterSearch:!!filters.search&&!demandOwnSearchMatch}),isExpanded=expanded.has(sourceDemand.id),editors=editMode?demandCellEditors(demand,settings,people):null;
