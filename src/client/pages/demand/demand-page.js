...
export class DemandPage{
  constructor({gateway,elements,settings,workspace,data,onStatus,onEditModeChange,initialFilters={},onFiltersChange}){
    this.gateway=gateway;this.elements=elements;this.settings=settings;this.workspace=workspace;this.data=data;this.onStatus=onStatus;this.onEditModeChange=onEditModeChange;this.onFiltersChange=onFiltersChange;
    this.filters=new DemandFilterState(initialFilters);this.scope={mode:'all'};this.expanded=new Set();this.queryService=this.buildQueryService();this.editMode=false;this.demandDrafts=new Map();this.workPackageDrafts=new Map();this.changedDemandIds=new Set();this.changedWorkPackageIds=new Set();
  ...
  applyFilter(key,value){this.filters.set(key,value);if(key==='control'&&value==='work-item-missing')for(const d of this.queryService.query(this.filters.value,this.scope))this.expanded.add(d.id);this.onFiltersChange?.(this.filters.value);this.render()}
  clearFilters(){this.filters.reset();this.onFiltersChange?.(this.filters.value);this.render()}
  render(){
    const filters=this.filters.value,rows=this.queryService.query(filters,this.scope);
    renderDemandFilterBar(this.elements.filters,{filters,settings:this.settings,people:this.data.people,onChange:(key,value)=>this.applyFilter(key,value),onClear:()=>this.clearFilters()});
    renderDemandTable(this.elements.table,{demands:rows,queryService:this.queryService,settings:this.settings,people:this.data.people,expanded:this.expanded,filters,editMode:this.editMode,demandDraftFor:d=>this.demandDraftFor(d),workPackageDraftFor:w=>this.workPackageDraftFor(w),onInlineChange:(type,id,field,value)=>this.inlineChange(type,id,field,value),onToggle:id=>{this.expanded.has(id)?this.expanded.delete(id):this.expanded.add(id);this.render()},onEdit:d=>this.openEditor(d),onEditWorkPackage:(wp,d)=>this.openWorkPackageEditor(wp,d)});
  ...
  start(){this.scopeSelector.render();if(this.filters.value.control==='work-item-missing')for(const d of this.queryService.query(this.filters.value,this.scope))this.expanded.add(d.id);this.render()}
}
