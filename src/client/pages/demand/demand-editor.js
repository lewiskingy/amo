...
export async function editDemand(dialog,{demand,settings,people,onSave,isNew=false}){
  ...
  const teamId=demand.teamId||demand.owningTeamId||'';
  dialog.setAttribute('aria-labelledby','demand-editor-title');
  dialog.innerHTML=`<form method="dialog" class="demand-editor"><header><div><h2 id="demand-editor-title">${isNew?'New Demand':esc(demand.id)}</h2><p>${isNew?`Create ${esc(demand.id)} in the canonical /demand slice.`:'Edit Defined Demand in the canonical /demand slice.'}</p></div><button class="btn" value="cancel">Close</button></header><div class="editor-grid">
    ...
    <label>Owning Team<select name="teamId">${option('','Unassigned',teamId)}${(settings.teams||[]).map(team=>option(team.id,team.name||team.id,teamId)).join('')}</select></label>
  ...
  </div><footer><span data-error class="editor-error" role="alert" aria-live="polite"></span><button type="button" class="btn primary" data-save>${isNew?'Create Demand':'Save Demand'}</button></footer></form>`;
  ...
  business.addEventListener('change',()=>{working.initiative='';renderInitiatives()});renderInitiatives();dialog.showModal();form.elements.title.focus();
