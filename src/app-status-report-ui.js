...
(function initStatusReportUi(){
  ...
  function publicationTargetMarkup(){
    const mode=reportSourceMode(),publication=window.AmoStatusReportPublication;
    if(mode!=='local')return '<div class="status-publication-control status-publication-remote"><span class="status-publication-caption">Remote publication</span><strong>JSON snapshot only</strong><span class="status-publication-format">PDF not enabled</span></div>';
    const info=publication?.localTargetInfo?.()||{loaded:false,selected:false,name:''},name=info.selected?(info.name||'Selected folder'):(info.loaded?'Not selected':'Loading…'),action=info.selected?'Change…':'Choose…';
    return `<div class="status-publication-control"><span class="status-publication-caption">Publish HTML to</span><strong class="status-publication-value" title="${escHtml(name)}">${escHtml(name)}</strong><button type="button" class="btn status-publication-change" ${info.loaded?'':'disabled'}>${action}</button></div>`
  }
  function decorateLifecycleActionHints(){
    const hints={publishStatusDraft:'Publish the current Draft. Local Workspace also writes a self-contained HTML report to the selected publication folder.',unpublishStatusReport:'Reopen this Published report as Draft. Existing published files are retained.',newStatusDraft:'Finalise this Published report and start the next reporting cycle.',viewCurrentPublished:'View the current Published report.',editStatusDraft:'Edit the current Draft.',previewStatusDraft:'Preview the current Draft without publishing it.'};
    for(const[id,title]of Object.entries(hints)){const button=$(id);if(button)button.title=title}
  }
  function decoratePublicationControls(){
    const toolbar=$('statusReportToolbar');if(!toolbar)return;toolbar.classList.add('status-report-toolbar');decorateLifecycleActionHints();
    let control=toolbar.querySelector('.status-publication-control');if(control)control.remove();toolbar.insertAdjacentHTML('beforeend',publicationTargetMarkup());control=toolbar.querySelector('.status-publication-control');
    const change=control?.querySelector('.status-publication-change');if(change)change.addEventListener('click',async()=>{
      const publication=window.AmoStatusReportPublication;if(!publication?.changeLocalTarget)return;
      change.disabled=true;const oldText=change.textContent;change.textContent='Selecting…';
      try{await publication.changeLocalTarget()}catch(e){alert(`Could not change publication folder: ${e.message}`)}finally{if(change.isConnected){change.disabled=false;change.textContent=oldText}decoratePublicationControls()}
    })
  }
  ...
  function focusAuthoringPage(){
    ...
    decoratePublicationControls();decorateAuthoringContext();renderLatestReportCard()
  }
  ...
  window.addEventListener?.('amo:status-report-publication-target',()=>decoratePublicationControls());

  ensureRendererStyles();focusAuthoringPage();
  const css=document.createElement('style');css.id='status-report-ui-styles';css.textContent=`.status-modal{width:min(1180px,96vw)}.status-modal-toolbar{position:sticky;top:0;z-index:5;display:flex;justify-content:space-between;align-items:flex-end;gap:12px;padding:10px 0 14px;background:var(--panel);border-bottom:1px solid var(--line);margin-bottom:14px}.status-scope-controls{display:flex;gap:10px;flex-wrap:wrap}.status-scope-controls label{display:flex;flex-direction:column;gap:5px;font-size:.78rem;font-weight:700;color:var(--muted)}.status-scope-controls select{min-width:210px;border:1px solid var(--line);border-radius:8px;padding:7px 28px 7px 9px;background:var(--panel);color:var(--ink)}.latest-status-report-card .toolbar{margin:0}.status-report-toolbar{display:flex!important;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap}.status-publication-control{display:inline-flex;align-items:center;gap:7px;margin-left:4px;padding-left:11px;border-left:1px solid var(--line);min-height:32px;font-size:.76rem;white-space:nowrap}.status-publication-caption{color:var(--muted);font-weight:700}.status-publication-value{max-width:190px;overflow:hidden;text-overflow:ellipsis}.status-publication-change{padding:5px 8px;font-size:.72rem}.status-publication-remote{color:var(--muted)}.status-publication-remote strong{color:var(--ink)}.status-publication-format{padding:2px 6px;border-radius:999px;background:var(--soft);font-size:.68rem;font-weight:700;color:var(--muted)}.status-context-label{font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.04em;color:var(--muted)}.status-commentary-cell{vertical-align:top!important}.status-commentary-stack{min-height:150px;height:100%;display:flex;flex-direction:column;gap:8px}.status-commentary-stack>textarea[data-status-field]{margin-top:auto;min-height:76px;flex:0 0 auto}.status-previous-context{margin:0;padding:8px;border:1px solid var(--line);border-radius:8px;background:var(--soft)}.status-previous-head{display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin-bottom:5px}.status-previous-text{white-space:pre-wrap;font-size:.78rem;line-height:1.4;color:var(--ink)}.status-copy-previous{width:24px;height:24px;display:inline-flex;align-items:center;justify-content:center;flex:0 0 auto;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--muted);cursor:pointer;padding:4px}.status-copy-previous:hover{color:var(--ink);border-color:var(--accent)}.status-copy-previous.copied{color:var(--success)}.status-copy-previous svg{width:15px;height:15px;fill:none;stroke:currentColor;stroke-width:1.8}.status-work-package-context{margin-top:9px;padding-top:8px;border-top:1px solid var(--line);display:grid;gap:5px}.status-work-package-row{display:grid;grid-template-columns:auto minmax(120px,1fr) auto;gap:6px;align-items:center;font-size:.72rem}.status-work-package-row .muted{grid-column:2/-1}.status-report-table .previous-report-detail,.status-report-table .previous-copy-all{display:none!important}@media(max-width:900px){.status-publication-control{flex-basis:100%;justify-content:flex-end;border-left:0;border-top:1px solid var(--line);padding:7px 0 0;margin:0}}@media(max-width:760px){.status-modal-toolbar{align-items:stretch;flex-direction:column}.status-scope-controls{display:grid;grid-template-columns:1fr}.status-scope-controls select{width:100%;min-width:0}.status-work-package-row{grid-template-columns:1fr}.status-report-toolbar{justify-content:flex-start}.status-publication-control{justify-content:flex-start;white-space:normal}.status-publication-value{max-width:150px}}`;document.head.appendChild(css)
})();
