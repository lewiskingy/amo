...
(function initStatusHealthSync(){
  ...
  function normalizeHealthModel(){
    if(!workspaceHandle||!db?.workspace||!db?.settings)return;
    let changed=false;
    ...
    const normalizeEntry=e=>{const next=normalizeHealth(e?.health||e?.rag);if(next)e.health=next;if(Object.prototype.hasOwnProperty.call(e,'rag'))delete e.rag};
  ...
  function healthOptions(selected,includeAll=false){return `${includeAll?'<option value="">All</option>':'<option value="">Unset</option>'}${HEALTH_STATES.map(v=>`<option value="${escHtml(v)}" ${v===selected?'selected':''}>${escHtml(v)}</option>`).join('')}`}
  function applyHealthUi(){
    ...
    document.querySelectorAll('#statusReportTable tr[data-status-demand]').forEach(tr=>{
      ...
      if(statusReportState.editing){
        ...
        if(select){select.innerHTML=healthOptions(health,false);select.value=health;if(!select.dataset.healthSyncBound){select.dataset.healthSyncBound='true';select.addEventListener('change',()=>setTimeout(applyHealthUi,0))}}
        cell.querySelector('.health-sync-note')?.remove();
        if(healthIsOverride(d,explicit)){
          const note=document.createElement('span');note.className='health-sync-note';note.textContent=' *';note.title='Will update Demand Health when this Status Report is published.';note.setAttribute('aria-label',note.title);cell.appendChild(note)
        }
  ...
  function applyHealthConfigSemantics(){
    ...
    card.querySelector('[data-config-add]')?.remove();card.querySelectorAll('[data-config-input]').forEach(i=>i.disabled=true);card.querySelectorAll('[data-config-delete]').forEach(b=>b.remove())
  }
  ...
  const baseRenderRecordModalHealth=renderRecordModal;
  renderRecordModal=function(){
    const r=baseRenderRecordModalHealth();
    if(recordModalState.type==='config'&&recordModalState.configKey==='healthStates'){
      $('recordModalSubtitle').textContent='Controlled Health state';
      $('recordModalActions')?.querySelector('[data-modal-edit]')?.remove()
    }
    const health=document.querySelector('#recordModalBody [data-modal-field="health"]');if(health)health.title=HEALTH_HELP[health.value]||'Demand Health';
    return r
  };
  ...
  const style=document.createElement('style');style.id='status-health-sync-styles';style.textContent=`.health-sync-note{font-weight:800;color:var(--warn);cursor:help;margin-left:2px}.health-green{background:#1b7f5a}.health-amber{background:#d88a00}.health-red{background:#b42318}.health-unset{background:#98a2b3}html[data-theme="dark"] .health-sync-note{color:var(--warn)}`;document.head.appendChild(style);
})();
