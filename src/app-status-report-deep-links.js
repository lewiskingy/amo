...
(function initStatusReportDeepLinks(){
  ...
  function decorateLatest(){
    const host=$('latestStatusReport'),report=typeof latestReport==='function'?latestReport():null;if(!host||!persistedReport(report)||host.querySelector('[data-open-latest-report]'))return;const mode=reportSourceMode(),flex=host.querySelector('.flex');if(!flex)return;
    const actions=document.createElement('div');actions.className='toolbar';const open=document.createElement('button');open.className='btn';open.dataset.openLatestReport=report.id;open.textContent='Open Report';open.addEventListener('click',()=>openReportPage(report.id,mode));const copy=document.createElement('button');copy.className='btn';copy.textContent=mode==='local'?'Copy Local Link':'Copy Link';copy.addEventListener('click',()=>copyReportLink(report.id,mode,copy));const existing=flex.querySelector('#viewLatestStatus');if(existing){existing.replaceWith(actions);actions.append(existing,open,copy)}else{actions.append(open,copy);flex.append(actions)}
  }
