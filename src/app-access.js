...
(function initAmoAccess(){
  ...
  function resolveUser(identity=currentAuthIdentity()){
    ...
    return users().find(user=>identitiesFor(user).some(binding=>{
      if(String(binding.provider||'').toLowerCase()!==provider)return false;
      const configuredSubject=String(binding.subject||binding.providerSubject||binding.objectId||'');
      if(configuredSubject&&subject)return configuredSubject===subject;
      return !!email&&normalizeEmail(binding.email||binding.providerEmail||binding.username)===email
    }))||null
  }
  function resolveIdentityBinding(identity=currentAuthIdentity()){
    const user=resolveUser(identity);if(!user||!identity)return null;
    const provider=String(identity.provider||'').toLowerCase(),subject=String(identity.subject||identity.objectId||''),email=normalizeEmail(identity.email||identity.username);
    return identitiesFor(user).find(binding=>{
      if(String(binding.provider||'').toLowerCase()!==provider)return false;
      const configuredSubject=String(binding.subject||binding.providerSubject||binding.objectId||'');
      if(configuredSubject&&subject)return configuredSubject===subject;
      return !!email&&normalizeEmail(binding.email||binding.providerEmail||binding.username)===email
  ...
  function currentPrincipal(){
    ...
    if(!identity)return{authenticated:false,mapped:false,kind:'anonymous',id:'anonymous',displayName:'Anonymous',companyAccount:'',identity:null,user:null,binding:null};
    const user=resolveUser(identity),binding=user?resolveIdentityBinding(identity):null,companyAccount=String(user?.companyAccount||user?.entraAccount||user?.workAccount||'').trim();
    return{
      authenticated:true,
      mapped:!!user,
      kind:user?'mapped':'authenticated',
      id:String(user?.id||companyAccount||identity.subject||identity.email||'authenticated-user'),
      displayName:String(user?.displayName||identity.name||companyAccount||identity.email||'Authenticated user'),
      companyAccount,
      identity,
      user,
      binding
  ...
  function writeAllowed(){const principal=currentPrincipal();if(!principal.authenticated)return false;return accessConfig().writeRequiresMapping===true?principal.mapped:true}
  ...
  function requireCapability(capability,message){
    ...
    throw new Error(message||(needsMapping?'Your signed-in identity is not mapped to an approved AMO User.':'AMO is read-only until you sign in with Google.'))
  }
  function accessMode(){return writeAllowed()?'read-write':'read-only'}
  function notify(){const state={mode:accessMode(),principal:currentPrincipal()};listeners.forEach(fn=>{try{fn(state)}catch(_e){}});try{window.dispatchEvent(new CustomEvent('amo-access-changed',{detail:state}))}catch(_e){}}

  function mutationButton(button){
    if(!(button instanceof HTMLElement))return false;if(button.closest('.amo-auth-profile'))return false;if(button.dataset.amoWrite==='true')return true;
    const id=String(button.id||'');if(['saveWorkspaceBtn','newDemandBtn'].includes(id))return true;
  ...
  function setAccessDisabled(element,disabled){if(disabled){if(!element.disabled){element.disabled=true;element.dataset.amoAccessDisabled='true';element.title=element.title||'Sign in with Google to make changes.'}}else if(element.dataset.amoAccessDisabled==='true'){element.disabled=false;delete element.dataset.amoAccessDisabled}}
  function applyModalReadOnly(readOnly){
    const modal=document.getElementById('recordModalBody');if(!modal)return;
    modal.querySelectorAll('input,select,textarea,button,[contenteditable="true"]').forEach(el=>{if(el.dataset.amoReadonlyAllow==='true')return;if(el.matches('button')&&!mutationButton(el))return;if('disabled'in el)setAccessDisabled(el,readOnly);else if(readOnly){el.dataset.amoAccessContenteditable=el.getAttribute('contenteditable')||'true';el.setAttribute('contenteditable','false')}else if(el.dataset.amoAccessContenteditable){el.setAttribute('contenteditable',el.dataset.amoAccessContenteditable);delete el.dataset.amoAccessContenteditable}})
  }
  function renderAccessBadge(readOnly){
    const host=document.querySelector('.workspace-banner .flex');if(!host)return;let badge=document.getElementById('amoAccessMode');if(!badge){badge=document.createElement('span');badge.id='amoAccessMode';badge.className='pill';host.appendChild(badge)}
    const principal=currentPrincipal();badge.className=`pill ${readOnly?'':'green'}`;badge.textContent=readOnly?(principal.authenticated?'Read only · user not approved':'Read only · sign in to edit'):'Edit enabled';badge.title=principal.companyAccount?`Acting as ${principal.companyAccount}`:(principal.identity?.email||'')
  }
  function applyUiState(){refreshQueued=false;const readOnly=!can(CAPABILITIES.workspaceWrite);document.documentElement.dataset.amoAccessMode=readOnly?'read-only':'read-write';document.querySelectorAll('button').forEach(button=>{if(mutationButton(button))setAccessDisabled(button,readOnly)});applyModalReadOnly(readOnly);renderAccessBadge(readOnly)}
  ...
  function guardLocalRepository(){
    ...
    p.deletePath=async function(...args){requireCapability(CAPABILITIES.workspaceWrite);return originalDelete.apply(this,args)};p.__amoAccessGuarded=true
  }
  function guardRemoteInstance(repo){
    ...
    repo.request=async function(path,options={}){const method=String(options.method||'GET').toUpperCase();if(!['GET','HEAD','OPTIONS'].includes(method))requireCapability(CAPABILITIES.workspaceWrite);return original(path,options)};repo.__amoAccessGuardedRequest=true;return repo
  }
  function guardRepositorySelection(){
    guardLocalRepository();guardRemoteInstance(window.workspaceRepository);
    if(typeof window.setWorkspaceRepository==='function'&&!window.setWorkspaceRepository.__amoAccessGuarded){const original=window.setWorkspaceRepository;const guarded=function(repo){return original(guardRemoteInstance(repo))};guarded.__amoAccessGuarded=true;window.setWorkspaceRepository=guarded}
  }
  function guardGlobalWrites(){
    if(typeof window.ensureRW==='function'&&!window.ensureRW.__amoAccessGuarded){const original=window.ensureRW;const guarded=async function(...args){if(!can(CAPABILITIES.workspaceWrite))return false;return original.apply(this,args)};guarded.__amoAccessGuarded=true;window.ensureRW=guarded}
    if(typeof window.writeJson==='function'&&!window.writeJson.__amoAccessGuarded){const original=window.writeJson;const guarded=async function(...args){requireCapability(CAPABILITIES.workspaceWrite);return original.apply(this,args)};guarded.__amoAccessGuarded=true;window.writeJson=guarded}
  ...
  window.amoAccess={CAPABILITIES,can,require:requireCapability,mode:accessMode,currentPrincipal,users,resolveUser,resolveIdentityBinding,config:accessConfig,refresh:()=>{installGuards();scheduleUiRefresh();notify()},onChange};
