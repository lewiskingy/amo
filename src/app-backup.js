...
async function pruneWorkspaceBackups(value=window.workspaceRepository,now=new Date()){
  ...
  const before=entries.length;await repo.pruneBackups({keep});const removed=before-keep.size;if(removed>0)log(`Backup retention removed ${removed} superseded backup${removed===1?'':'s'}.`)
}
