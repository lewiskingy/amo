...
(function initRemoteRecovery(){
  ...
  async function renderRemoteRestore(){
    ...
    try{
      ...
      else{html+='<div class="table-wrap"><table><thead><tr><th>Point</th><th>Type</th><th>Actor / detail</th><th>Documents</th><th></th></tr></thead><tbody>';for(const p of points)html+=`<tr><td>${esc(new Date(p.timestamp).toLocaleString('en-GB'))}</td><td>${esc(p.type||'Change')}</td><td>${esc(p.actor||'')}${p.reason?` · ${esc(p.reason)}`:''}</td><td>${Number(p.changed)||0}</td><td><button class="btn" data-remote-restore="${esc(p.id)}">Preview Restore</button></td></tr>`;html+='</tbody></table></div>'}
