...
assert.match(ui,/source\.previousEntries=clone\(previous\.entries\|\|\[\]\)/);
...
// Commentary alignment must preserve native table columns: flex belongs inside each td, never on the td itself.
assert.match(ui,/function commentaryStack/);
assert.match(ui,/status-commentary-stack/);
assert.match(ui,/stack\.insertBefore\(box,textarea\)/);
assert.match(ui,/\.status-commentary-cell\{vertical-align:top!important\}/);
assert.doesNotMatch(ui,/\.status-commentary-cell\{[^}]*display:flex/,'Status Report td elements must remain table cells');
assert.match(ui,/\.status-commentary-stack\{[^}]*display:flex/);
assert.match(ui,/margin-top:auto/,'Current draft inputs should align at the bottom of their commentary stack');
...
assert.doesNotMatch(ui,/e\[field\]=previous/,'Clipboard copy must not write previous commentary into current commentary');
