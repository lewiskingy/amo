...
(async()=>{const server=spawn(process.execPath,['tests/e2e/bin/serve-target-client.cjs'],{stdio:['ignore','pipe','inherit']});try{
  ...
  const groups=page.locator('details.amo-shell-nav-group');assert.equal(await groups.count(),4,'Target navigation should expose the same four collapsible information-architecture groups as legacy AMO.');assert.deepEqual(await groups.locator('summary').allTextContents(),['Work','Planning & Reporting','Administration','Help']);
  const workGroup=page.locator('details[data-amo-nav-section="work"]');assert.equal(await workGroup.getAttribute('open'),'','The active Demand route should open the Work group.');const planningGroup=page.locator('details[data-amo-nav-section="planning"]');assert.equal(await planningGroup.getAttribute('open'),null);await planningGroup.locator('summary').click();assert.equal(await planningGroup.getAttribute('open'),'','Navigation section summaries must expand their groups.');
  const primaryDemand=workGroup.locator('a[href="/demand"]');assert.equal(String(await primaryDemand.textContent()).trim(),'Demand','The canonical /demand route should now be presented as Demand, not Preview.');assert.equal(await primaryDemand.getAttribute('class'),'active');
  const legacyDemand=workGroup.locator('a[href="/?view=demand"]');assert.equal(String(await legacyDemand.textContent()).trim(),'Demand (legacy)','The legacy Demand implementation must remain immediately available in Work as a fallback link.');
  assert.equal(await page.locator('summary:text-is("Management")').count(),0,'Target navigation must not retain a separate Management section.');

  const themeToggle=page.locator('[data-shell-theme]');assert.equal(await themeToggle.count(),1,'Every target page header should expose the shared light/dark toggle.');const initialTheme=await page.evaluate(()=>document.documentElement.dataset.theme);await themeToggle.click();const toggledTheme=await page.evaluate(()=>document.documentElement.dataset.theme);assert.notEqual(toggledTheme,initialTheme,'Theme toggle must change the active theme.');assert.equal(await page.evaluate(()=>localStorage.getItem('amo.theme')),toggledTheme,'Theme selection must persist using the shared browser preference.');await page.reload();await page.waitForSelector('.demand-row');assert.equal(await page.evaluate(()=>document.documentElement.dataset.theme),toggledTheme,'Persisted theme must rehydrate after page reload.');
